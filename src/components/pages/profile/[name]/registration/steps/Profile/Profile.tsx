...
const SubmitButton = ({
  ...
  const hasEthRecord = records.some((record) => record.key === 'lyx' && record.value === address)
...
const Profile = ({ name, callback, registrationData, resolverExists }: Props) => {
  ...
  const handleDeleteRecord = (record: ProfileRecord, index: number) => {
    if (record.key === 'lyx') return setModalOption('clear-eth')
      ...
      case 'clear-eth': {
        return (
          <ConfirmationDialogView
            ...
            onConfirm={() => {
              removeRecordByTypeAndKey('address', 'lyx')
              setModalOpen(false)
            }}
            onDecline={() => setModalOpen(false)}
          />
        ...
        {records.map((field, index) =>
          field.group === 'custom' ? (
            ...
            <ProfileRecordInput
              ...
              recordKey={field.key}
              group={field.group}
              disabled={field.key === 'lyx' && registrationData.reverseRecord}
              label={labelForRecord(field)}
              secondaryLabel={secondaryLabelForRecord(field)}
              placeholder={placeholderForRecord(field)}
              error={errorForRecordAtIndex(index)}
